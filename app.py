...
# Para resolverlo, se convirtio el shapefile con el departamento asociado a un geojson utilizando el siguiente codigo:
#
#   import geopandas as gpd
...
#
# Esto generará un archivo "antioquia.json" mucho más liviano que se utilizo para el github
# Algo similar se realizo en la actividad de shiny 
# ===========================================================
...
def update_summary(_):
    resumen = {
        ...
        "Rango de años": f"{df['ano'].min()} - {df['ano'].max()}" if "ano" in df.columns else "No disponible"
    }
    return str(resumen)
...
def update_map(colorPalette, show_labels, mapClass):
    ...
    resumen = df.groupby("codigo_municipio").agg(
        casos_totales=("numero_casos", "sum"),
    ...
    antioquia_data = antioquia.merge(
        resumen, left_on="mpio_cdpmp", right_on="codigo_municipio", how="left"
    )
